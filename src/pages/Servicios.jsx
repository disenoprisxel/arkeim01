...
const services = [
  {
    ...
    points: ['Anteproyecto y concepto', 'Planos arquitectónicos y técnicos', 'Gestión de permisos y licencias', 'Coordinación con ingeniería'],
    img: ARCH2,
  },
  {
...
export default function Servicios() {
  return (
    <div style={{ backgroundColor: '#0A0A0A', minHeight: '100vh' }}>
      ...
      {/* ── SERVICE BLOCKS ── */}
      {services.map((s, i) => (
        <section key={s.num} style={{ backgroundColor: '#0A0A0A', padding: '0 100px', display: 'flex', alignItems: 'center', gap: 0, minHeight: 900, flexDirection: i % 2 === 0 ? 'row' : 'row-reverse' }} className="srv-block">

          {/* Text side */}
          <div style={{ flex: '0 0 640px', display: 'flex', flexDirection: 'column', gap: 32, padding: '80px 0', paddingRight: i % 2 === 0 ? 64 : 0, paddingLeft: i % 2 !== 0 ? 64 : 0 }} className="srv-text">
            ...
            <RevealSection delay={0} direction={i % 2 === 0 ? 'left' : 'right'}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 20 }}>
                  <span style={{ fontFamily: 'Syne, sans-serif', fontSize: 80, fontWeight: 800, color: 'rgba(255,255,255,0.04)', letterSpacing: -4, lineHeight: 1 }}>{s.num}</span>
                  <RevealText tag="h2" delay={0.1} style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 700, color: '#fff', letterSpacing: -1, margin: 0, lineHeight: 1.1 }}>
                    {s.title}
                  </RevealText>
                </div>
              </div>
            ...
            <RevealSection delay={0.2} direction={i % 2 === 0 ? 'left' : 'right'}>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 16, color: 'rgba(255,255,255,0.55)', lineHeight: 1.7 }}>{s.desc}</p>
            </RevealSection>
            ...
            <RevealSection delay={0.55} direction={i % 2 === 0 ? 'left' : 'right'}>
